...
class Solution {
    ...
    private long rec(int index, int i, int[] nums, long[] prefixNums, long[] prefixCost, int k, long[][] memo, int[] bestSplit) {
        int n = nums.length;
        if (index == n) return 0;
        ...
        if (bestSplit[index] != -1) {
            ...
            long val = (prefixNums[j] + (long) k * i) * (prefixCost[j + 1] - prefixCost[index]) + rec(j + 1, i + 1, nums, prefixNums, prefixCost, k, memo, bestSplit);
