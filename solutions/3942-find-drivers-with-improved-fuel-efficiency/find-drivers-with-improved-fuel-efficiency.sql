WITH stat AS (
    ...
    SELECT driver_id, driver_name,
        frst_tot_fuel_ef/frst_tot_trips AS frst_half_avg,
        sec_tot_fuel_ef/sec_tot_trips AS sec_half_avg
    FROM stat
    INNER JOIN drivers USING(driver_id)
    WHERE stat.frst_tot_trips IS NOT NULL AND stat.sec_tot_trips IS NOT NULL
)
